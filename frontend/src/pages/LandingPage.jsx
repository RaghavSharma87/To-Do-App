...
function Landing() {
  ...
  return (
    <div
      ...
      style={{ backgroundImage: waveBackground, backgroundSize: "cover", backgroundPosition: "center" }}
    >
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-32 lg:pb-8 flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="mb-10 max-w-lg">
              ...
              {[
                { title: "Drag & drop task ordering", meta: "Reorder your priorities effortlessly" },
                { title: "Category organisation", meta: "Group tasks into meaningful buckets" },
                { title: "Priority levels", meta: "High · Medium · Low — always know what's next" },
                { title: "Archive & Bin", meta: "Keep your workspace clean without losing history" },
                { title: "Dark & Light mode", meta: "Works beautifully in any light" },
              ].map((item, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 py-4 border-b border-border-subtle last:border-b-0"
                >
                  <div className="mt-0.5 w-[18px] h-[18px] rounded-full border-2 border-border-subtle flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold font-serif leading-snug text-text-main">
                      {item.title}
                    </p>
                    <p className="text-xs text-text-muted font-sans mt-0.5">{item.meta}</p>
                  </div>
                </div>
              ))}
            </div>
