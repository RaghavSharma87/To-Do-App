...
function MissedDeadlines() {
  ...
  const overdueText = (date, time) => {
    ...
    return `${Math.max(1, hours)} hour(s)`;
  };
    ...
    <div className="min-h-screen flex bg-background/60">
      <aside className="hidden lg:flex w-56 border-r border-border-subtle px-5 py-8 flex-col bg-background/80">
        <button
          ...
          className="flex items-center gap-2 text-xs text-text-muted hover:text-text-main mb-4"
        >
          <ArrowBigLeft size={14} /> Back Home
        </button>
        <p className="text-sm font-bold">Missed Deadlines</p>
        <p className="text-xs text-text-muted mt-2">
          Recover your efficiency by completing overdue work.
        </p>
      </aside>
        ...
        {missedTasks.length === 0 ? (
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[10px] uppercase tracking-[0.12em] font-semibold">
                Overdue Tasks
              </h3>
            ...
            <AnimatePresence>
              {missedTasks.map((task) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="flex items-start gap-3 py-4 border-b border-border-subtle group"
                >
                  <button
                    onClick={() => handleComplete(task.id)}
                    className="mt-0.5 w-[18px] h-[18px] rounded-full border-2 border-red-400 hover:bg-green-500 hover:border-green-500 transition-all"
                  />
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-muted mt-0.5">
                      ...
                      {task.end_time && `• ${task.end_time}`} • Overdue by{" "}
